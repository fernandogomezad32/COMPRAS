...
export function ProductManagement() {
  ...
  const loadUserRole = async () => {
    try {
      ...
      console.log('🔍 [ProductManagement] User role loaded:', role);
      setUserRole(role);
    } catch (error) {
      console.error('Error loading user role:', error);
      console.log('🚨 [ProductManagement] Defaulting to employee role due to error');
  ...
  // Para empleados, mostrar mensaje informativo
  if (userRole === 'employee') {
    return (
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600 mt-1">Consulta información de productos disponibles</p>
          </div>
          <div className="bg-blue-50 px-4 py-2 rounded-lg">
            <p className="text-sm text-blue-700">👁️ Solo consulta - Sin permisos de edición</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-md">
          ...
          {filteredProducts.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="mt-1 text-sm text-gray-500">No se encontraron productos con los filtros aplicados.</p>
            </div>
          )}
        </div>
      </div>
